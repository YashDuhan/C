//There seems to be an error with this code and is yet to be fixed
...
void stack::createstack(stack *stack, int st_size, int st_top)
{
    stack->size = st_size;
    stack->top = st_top;
    stack->arr = new char[st_size*sizeof(char)]; //created a stack with the desired size
}
...
bool stack::parenthesisMatch(stack *stack,char *exp)
{
    ...
    for(int i = 0; exp[i] != '\0';i++) //did \0 because the character arrays have this end character at the end
    {
        ...
        else if(exp[i] == ')' || exp[i] == '}' || exp[i] == ']')
        {
            if(isEmpty(stack))
            {
                return false; //check if underflow or stack has been initialized with closing parenthesis
            }
            popped_ch = pop(stack);
